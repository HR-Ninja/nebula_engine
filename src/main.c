...
void init_projection(const Shader_names shader);
...
int main() {
    ...
    init_projection(SHADER_DEFAULT);
    
    uint32_t location = glGetUniformLocation(shaders[SHADER_DEFAULT], "image");
    check_location(location);
    glUniform1i(location, 0);

    player_tag = register_component(sizeof(char));
    sprite_component = register_component(sizeof(Sprite));
    transform_component = register_component(sizeof(Transform));
    velocity_component = register_component(sizeof(Velocity));
...
void handle_input() {
    if (screen_has_resized) {
        ...
        if (is_key_pressed(GLFW_KEY_SPACE)) {
            ...
            Transform p = {
                .position = {(float)(screen_width / 2), (float)(screen_height / 2), 0.0f}
            };
            ...
            for (int i = 0; i < 49999; i++) {
                ...
                attach_component(e, sprite_component, &s);
...
void fixed_update() {
    while (accumulated_dt >= fixed_timestep) {
            // Collision here

            accumulated_dt -= fixed_timestep;    
        }
}
...
void move_system(Component transform_c, Component velocity_c) {
    ...
    for (Entity e = 0; e < MAX_ENTITIES; e++) {
        if (HAS_COMPONENT(entity_mask[e], transform_c) && HAS_COMPONENT(entity_mask[e], velocity_c)) {
            ...
            if (glm_vec3_eq_eps(vel->current, 0.001f) && glm_vec3_eq_eps(vel->target, 0.001f))
                    continue;
...
void player_controller(Entity e, Component velocity_c) {
    if(HAS_COMPONENT(entity_mask[e], velocity_c)) {
        ...
        if (glm_vec3_norm(vel->target) > 0.0f) {
            ...
            glm_vec3_scale(vel->target, vel->speed, vel->target);
        }
    }
}
...
void init_projection(const Shader_names shader) {
    ...
    use_shader(shaders[shader]);
    uint32_t location = glGetUniformLocation(shaders[shader], "projection");
    check_location(location);
